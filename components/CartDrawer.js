                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center justify-between mt-1.5">
                    <div className="flex items-center border rounded-full" style={{ borderColor: COLORS.line }}>
                      <button onClick={() => updateQty(idx, item.qty - 1)} aria-label={`Decrease quantity of ${item.product.name}`} className="p-2.5"><Minus size={12} /></button>
                      <span className="font-mono text-xs w-5 text-center" aria-live="polite">{item.qty}</span>
                      <button onClick={() => updateQty(idx, item.qty + 1)} aria-label={`Increase quantity of ${item.product.name}`} className="p-2.5"><Plus size={12} /></button>
                    </div>
                    <span className="font-mono text-sm font-semibold" style={{ color: COLORS.ink }}>{formatPKR(item.product.price * item.qty)}</span>
                  </div>
                </div>
                <button onClick={() => removeItem(idx)} aria-label={`Remove ${item.product.name} from cart`} className="self-start p-2.5 -mr-2.5 -mt-2"><Trash2 size={15} color={COLORS.muted} /></button>
              </div>
            );
          })}
        </div>
